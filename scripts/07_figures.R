...
training_ranges <- data.frame(
  expl.name = c("bio_12", "bio_10", "bio_8", "bio_2", "bio_9"),  
  training_min = c(753,24.338,14.579, 12.193, 2.209),        
  training_max = c(1378,27.876,24.408, 13.840, 25.821)           
)
...
response_curves <- response_curves %>%
  ...
  mutate(
    training_min = ifelse(is.na(training_min), -Inf, training_min),  
    training_max = ifelse(is.na(training_max), Inf, training_max)    
  )

response_curves$expl.name <- factor(
  response_curves$expl.name, 
  levels = c("bio_12", "bio_10", "bio_9", "bio_8", "bio_2", "percent_prairie", "percent_clay", "SDM.global")  
)
...
line_data <- data.frame(
  y = c(2149, 2516, 2370),
  line_type = c("Regional-only","Multiply", "Covariate")  
)
